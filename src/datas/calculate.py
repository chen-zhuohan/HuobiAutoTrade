...
from datas.query import Kline


def calculate_ma(data: List[Kline]):
    """ 计算周期的均线 """
    if len(data) != 20:
...
def calculate_LB(data: List[Kline]):
    """ 计算布林下轨 """
    if len(data) != 20:
        raise ValueError('calculate 20 period moving average should give 20 period data!')
...
def calculate_UB(data: List[Kline]):
    """ 计算布林上轨 """
    if len(data) != 20:
        raise ValueError('calculate 20 period moving average should give 20 period data!')
    last_20_weeks_close = [i.close for i in data]
...
def calculate_vol_vol3(data: List[Kline]):
    """ 计算"""
    if len(data) != 3:
